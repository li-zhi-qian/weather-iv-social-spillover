...
def get_res(selected_ownweather):
    ...
    data[outcome + '_wkn1d_r'] = data.groupby('opening_sat_date')[outcome+'_wkn1d_r'].transform('sum')
...
def ivreg_s2(selected):
    ...
    for i in range(2, 7):
        ...
        model_summary = iv_model.summary()
    ...
    # Please omit this
    filtered_data = data[data['wk1']==1]
    iv_model = do_ivreg(
        X=filtered_data[outcome + "_wk1d_r"], Y=filtered_data[f"{outcome}_wkn1d_r"], Z=filtered_data[selected]
    )
    model_summary = iv_model.summary()
    # print(filtered_data[outcome + "_wk1d_r"].describe())
    # print(filtered_data[outcome + "_wkn1d_r"].describe())
    # print(filtered_data[selected].describe())
    coefficients.append(f"{iv_model.params.iloc[1]:.3f}")
    std_errors.append(f"{iv_model.bse.iloc[1]:.4f}")
...
def ols():
    ...
    # Please omit this
    X=data[outcome+'_wk1d_r']
    y=data[f'{outcome}_wkn1d_r']
    X=sm.add_constant(X)
    ols_model = sm.OLS(y, X).fit()
    ols_coef.append(f'{ols_model.params.iloc[1]:.3f}')
    ols_sd.append(f'{ols_model.bse.iloc[1]:.4f}')
    ols_r2.append(r2_score(y,ols_model.predict(X)))
...
if __name__ == "__main__":
    ...
    instrument_dict = {'tickets_ratedgpg':"open_res_own_prec_0_6","tickets_adult":"open_res_own_mat5_75_0","tickets":"open_res_own_mat5_75_0"}
    outcome = "tickets_ratedgpg" # TODO: set dependent variable
    selected_instruments = [instrument_dict[outcome]] #TODO: set instruments["open_res_own_mat_la_cens_6"] #open_res_own_prec_0_6 "open_res_own_mat5_75_0"
    comments = "gpg_prec_iv" # TODO: additional stuffs in output file names open_res_own_mat5_75_0
    
    s1_model = ivreg_s1(selected=selected_instruments) # do 2sls stage 1 with the selected instrument(s)
    ...
    results, model_list= ivreg_s2(selected=selected_instruments)  # do stage 2 and retrieve coefficients and models
